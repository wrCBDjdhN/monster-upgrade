...
class WarehouseView(ScrollView):
    ...
    def on_draw(self):
        ...
        # 固定头部
        arcade.draw_text("仓 库", WINDOW_WIDTH // 2, WINDOW_HEIGHT - 50,
                         arcade.color.GOLD, 30, anchor_x="center")
        arcade.draw_text(f"金币: {gold}", WINDOW_WIDTH // 2, WINDOW_HEIGHT - 85,
        ...
        for item in wh:
            if item["item_type"] == "resource":
                ...
                arcade.draw_text(f"{name} x{item['quantity']}", 60, y,
        ...
        for btn, wh_id, gold_per, item_id in self.sell_buttons:
            ...
            arcade.draw_text("售卖", draw_btn.center_x, draw_btn.center_y,
                             arcade.color.WHITE, 11, anchor_x="center", anchor_y="center")

        # ── 武器列表 ──
        arcade.draw_text("武器:", 50, y - 20, arcade.color.WHITE, 16)
        y -= 50
        self.equip_buttons = []
        self.weapon_sell_buttons = []
        ...
        for w in weapons:
            ...
            arcade.draw_text("已装备" if is_equipped else "装备", eq_btn.center_x, eq_btn.center_y,
            ...
            arcade.draw_text(f"卖{sell_price}G", sv_btn.center_x, sv_btn.center_y,
                             arcade.color.WHITE, 10, anchor_x="center", anchor_y="center")
            y -= 32

        if not weapons:
            arcade.draw_text("(无武器，击杀怪物获取)", 60, y, arcade.color.GRAY, 12)

        # ── 装备列表 ──
        arcade.draw_text("装备:", 50, y - 20, arcade.color.WHITE, 16)
        y -= 50
        self.equip_item_buttons = []
        self.equip_sell_buttons = []
        equipment = get_equipment_inventory(pid)
        for eq in equipment:
            ...
            arcade.draw_rect_filled(eq_btn, eq_color)
            arcade.draw_text("已装备" if eq["is_equipped"] else "装备", eq_btn.center_x, eq_btn.center_y,
                             arcade.color.WHITE, 10, anchor_x="center", anchor_y="center")
            # 售卖按钮
        ...
        if not equipment:
            arcade.draw_text("(无装备)", 60, y, arcade.color.GRAY, 12)
            y -= 32
        ...
        arcade.draw_text("市场", self.market_rect.center_x, self.market_rect.center_y,
                         arcade.color.WHITE, 13, anchor_x="center", anchor_y="center")
